...
class SessionController extends Controller
{
    public function show(LearningUnit $unit): Response
    {
        ...
        return Inertia::render('session/show', [
            'unit' => ['id' => $unit->id, 'slug' => $unit->slug, 'title' => $unit->title, 'schedule' => $unit->metadata['schedule'] ?? null, 'estimatedMinutes' => $unit->estimated_minutes],
            'blocks' => $blocks->map(fn ($block): array => [
                'id' => $block->id, 'type' => $block->block_type, 'title' => $block->title, 'body' => $block->body, 'required' => $block->is_required,
